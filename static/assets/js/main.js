(function ($) {
	...
	// function preloader2() {
	//     var moveLeft = $(window).width() - $('#loading-center').width() * 0.5;
	//     var moveUp = $(window).height() - $('#loading-center').height() * 0.5;

	//     $('#loading-center').animate({
	//         left: -moveLeft,
	//         top: -moveUp
	//     }, 'slow').fadeTo('slow', 0, function() {
	//         $(this).hide();
	//     });
	// };
	...
	$(document).ready(function () {
		...
		logo.data('originalWidth', originalWidth);
		logo.data('originalHeight', originalHeight);
		logo.data('originalTop', logo.offset().top + originalHeight / 2);
		logo.data('originalLeft', logo.offset().left + originalWidth / 2);


		// Scale up and center the logo.
		logo.css({
			position: 'fixed',
			top: '50%',
			left: '50%',
			transform: 'translate(-50%, -50%) scale(2)', // Scale it up a bit.
			zIndex: 999999 // Make sure it's above other elements.
		});
	...
	$(window).on('load', function () {
		...
		logo.stop().animate({
			top: logo.data('originalTop'),
			left: logo.data('originalLeft'),
			width: logo.data('originalWidth'),
			height: logo.data('originalHeight'),
			dummy: 1 // This is a dummy property that we'll use to animate the scale transformation.
		}, {
			duration: 800,
			easing: 'swing',
			step: function (now, fx) {
				if (fx.prop === 'dummy') {
					...
					var scale = 2 - now; // We're animating from 2 to 1.
			...
			complete: function () {
				// Reset the styles applied for the preloader effect.
				$(this).css({
					position: '',
					transform: '',
					zIndex: ''
				...
				// Remove the fullscreen white background.
